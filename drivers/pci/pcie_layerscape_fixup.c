				     ...
				     u16 *offset, u16 *stride)
{
	...
	bool have_ari = false;
	int pos;
	struct udevice *pf_dev, *next;

	dm_pci_read_config16(dev, sriov_pos + PCI_SRIOV_TOTAL_VFS, &tmp16);
	...
	device_foreach_child_safe(pf_dev, next, dev->parent) {
		dm_pci_read_config16(pf_dev, sriov_pos + PCI_SRIOV_CTRL,
				     &tmp16);
		tmp16 &= ~PCI_SRIOV_CTRL_VF_EN;
...
skip_ari:
	...
	dm_pci_write_config16(dev, sriov_pos + PCI_SRIOV_NUM_VFS,
			      entry->num_vfs);
	...
	dm_pci_write_config16(dev, sriov_pos + PCI_SRIOV_NUM_VFS, 0);
}
