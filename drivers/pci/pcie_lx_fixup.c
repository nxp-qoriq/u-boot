 ...
 */
static int lx_pcie_next_lut_index(struct lx_pcie *pcie)
{
...
static int lx_pcie_next_streamid(struct lx_pcie *pcie)
{
 ...
 */
static void lx_pcie_lut_set_mapping(struct lx_pcie *pcie, int index, u32 devid,
				    u32 streamid)
{
 ...
 */
static void fdt_pcie_set_msi_map_entry(void *blob, struct lx_pcie *pcie,
 ...
 */
static void fdt_pcie_set_iommu_map_entry(void *blob, struct lx_pcie *pcie,
					 u32 devid, u32 streamid)
{
	u32 *prop;
	u32 iommu_map[4];
...
#error "No CONFIG_FSL_PCIE_COMPAT defined"
	...
	if (devid == 0) {
		fdt_setprop_inplace(blob, nodeoff, "iommu-map",
				    iommu_map, 16);
	} else {
...
static void fdt_fixup_pcie(void *blob)
{
	...
	for (pci_find_first_device(&dev);
		...
		streamid = lx_pcie_next_streamid(pcie);
		if (streamid < 0) {
			debug("ERROR: no stream ids free\n");
			continue;
		}

		index = lx_pcie_next_lut_index(pcie);
		if (index < 0) {
		...
		lx_pcie_lut_set_mapping(pcie, index, bdf >> 8,
					streamid);
...
void ft_pci_setup(void *blob, bd_t *bd)
{
	...
	list_for_each_entry(pcie, &lx_pcie_list, list)
		ft_pcie_lx_setup(blob, pcie);
