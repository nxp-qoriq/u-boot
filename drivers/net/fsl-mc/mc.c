...
#ifdef CONFIG_SYS_LS_MC_DRAM_SPB_OFFSET
	mc_spb_offset = CONFIG_SYS_LS_MC_DRAM_SPB_OFFSET;
#else
#error "CONFIG_SYS_LS_MC_DRAM_SPB_OFFSET not defined"
#endif

	// Read blob header and get size of SPB blob
	sp_blob = (struct sp_blob_header *)mc_spb_addr;
	spb_size = le32_to_cpu(sp_blob->length);
	if (spb_size > CONFIG_SYS_LS_MC_SPB_MAX_LENGTH) {
